...
base64.encode(url.format(parts), {string: true}, function(err,data){
card.face=data;
client.publish('owntracks/'+user+'/'+fakename+"/info", JSON.stringify(card), {retain:true});
...
fakeOwnTracks(user, content.username, {"_type":"location",lat: content.loc[0], lon: content.loc[1],tid:content.username.substring(0,2),t:"a", batt:content.extra.bat*100,tst:content.lastupd,conn:"w"})
if (sendc) { sendCard(user, content.username, content.icon)}
...
console.log(data)
client.publish('owntracks/'+user+'/'+fakename, JSON.stringify(data), {retain:true});
}

function createWS(key, user){
   console.log(user)
	var connection = new autobahn.Connection({url: 'wss://stackptr.com/ws', realm: 'stackptr',authid: '-1',authmethods: ["ticket"], onchallenge: loginToWamp(key)});

connection.onopen = function (session) {
console.log("websocket opened");

session.subscribe("com.stackptr.user", function(data1,data2){return updateUsers(user,data2)});
session.call('com.stackptr.api.userList' ).then(function(data) {updateUsers(user,{"msg": data[1].data} , true   )
});
}
